...
export default async function handler(req, res) {
  ...
  try {
    ...
    const prompt =
      body?.prompt || "wrap the bag of image 1 in the texture of image 2, keep realism";

    const bag = body?.bag;     // { mimeType, data } base64 (no prefix)
    const fabric = body?.fabric; // { mimeType, data }

    if (!bag?.data || !fabric?.data) {
      return res.status(400).json({ error: "Missing bag or fabric (base64) in JSON body" });
    }
        ...
        contents: [
          {
            ...
            parts: [
              ...
              { inlineData: { mimeType: fabric.mimeType || "image/jpeg", data: fabric.data } },
            ],
          },
        ],
